<script lang="js">
    ...
    onMount(() => {
        axios.get('https://python-secret.onrender.com/util/wad')
    ...
    onMount(async () => {
        const querySnapshot = await getDocs(collection(db, "webbed"));
    ...
    const sendmessage = async () => {
        const docRef = await addDoc(collection(db, "webbed"), {
            text: currentMessage
        });
