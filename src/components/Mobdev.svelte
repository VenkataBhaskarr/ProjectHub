<script lang="js">
    ...
    onMount(() => {
                axios.get('https://python-secret.onrender.com/util/mad')
    ...
    onMount(async () => {
        const querySnapshot = await getDocs(collection(db, "mobdev"));
    ...
    const sendmessage = async () => {
        const docRef = await addDoc(collection(db, "mobdev"), {
            text: currentMessage
        });
